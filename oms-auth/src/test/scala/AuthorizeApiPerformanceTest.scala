...
class AuthorizeApiPerformanceTest extends Simulation {
  ...
  val scn = scenario("Load testing for authorize api")

    .exec(
      http("Authorizing the request for demo user in the db")
        .get("/authorize")
