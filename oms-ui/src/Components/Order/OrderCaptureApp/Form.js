...
const orderSchema = yup.object().shape({
  ...
  type: yup
    .string()
    .oneOf(["market", "limit"], "Invalid type")
    .required("Required"),
  exchange: yup
    .string()
    .oneOf(["bse", "nse"], "Invalid exchange")
    .required("Required"),
...
class Form extends React.Component {
    ...
    orderSchema
      ...
      .finally(async () => {
        ...
        if (noErrors === true) {
          ...
          if (body.type === "market") body.targetPrice = null;
          try {
            let response = await OmsAxios.post(
              `${Properties.ORDER_SERVER_URL}/order`,
              body
            );

            // Scenario: Current Access token working
            if (response.created) {
  ...
  //ADDING FUNCTION TO LOAD CLIENTS AS USER TYPES
  ...
  render() {
    return (
      ...
      //     <form data-testid="order-capture-form">
      //       <ul>
      //         <br />
      ...
      >
        ...
        >
          ...
          <div className="card-body">
            <form
              data-testid="order-capture-form"
              className="order-capture-form"
            >
              <br />
              ...
              <div className="row justify-content-center ">
                <div className="form-group p-2 bd-highlight col-md-10 col-12">
                  {/* <select
                    ...
                    id="Stock"
                    onBlur={this.handleValidateOnBlur}
                    data-testid="stock-client-select"
                  > */}
                  <SearchBox
                    name="clientID"
                  ...
                  {/* <>
                      {this.state.clients.map((client, index) => (
                        <option value={client.clientId} key={index}>
                          {client.clientName} - {client.clientId}
                        </option>
                      ))}
                      <option value="default" key="-1">--Select client--</option>
                    </> */}
                  {/* </select> */}
                  {/* <input name="clientID" value="1" onChange={this.clientSearch} className="form-control" placeholder="Search for clients"/> */}
